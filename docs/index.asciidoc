...
In this simple example each event from JDBC has an identical structure. If they were different, for example the first had a `language` field but the second did not, the data from prior events would need to be copied onto the `map`. 

For example:

[source,json]
----------------------------------
  { "country_name": "France", "town_name": "Paris", "language": "French" }
  { "country_name": "France", "town_name": "Marseille" }
----------------------------------

would be copied with `map['language'] ||= event.get('language')` in the aggregate code block.
